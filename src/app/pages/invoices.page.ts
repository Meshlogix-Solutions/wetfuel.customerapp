...
<wf-customer-shell title="Invoices" subtitle="Billing and payment history" backRoute="/home" [showNav]="true">
...
<ion-segment [(ngModel)]="filter"><ion-segment-button value="all"><ion-label>All</ion-label></ion-segment-button><ion-segment-button value="open"><ion-label>Open</ion-label></ion-segment-button><ion-segment-button value="paid"><ion-label>Paid</ion-label></ion-segment-button></ion-segment>
@for(invoice of visibleInvoices();track invoice.id){<ion-card class="wf-card" [routerLink]="['/invoice-detail',invoice.id]"><ion-card-content><div class="row-between"><span class="pill" [class.danger]="invoice.status==='sent'&&invoice.isOverdue" [class.warning]="invoice.status==='sent'&&!invoice.isOverdue" [class.success]="invoice.status==='paid'">{{invoice.status==='sent'?(invoice.isOverdue?'Overdue':'Open'):'Paid'}}</span><strong>{{invoice.invoiceNumber}}</strong></div><h2>{{invoice.total|currency}}</h2><p class="caption">{{invoice.lineItems.length}} deliver{{invoice.lineItems.length===1?'y':'ies'}}</p><div class="detail-row"><span>Issued {{invoice.issueDate|date:'mediumDate'}}</span><strong>Due {{invoice.dueDate|date:'mediumDate'}}</strong></div></ion-card-content></ion-card>}
...
export class InvoicesPage {
  ...
  filter='all';
  readonly outstandingTotal=computed(()=>this.invoices().filter(x=>x.status==='sent').reduce((sum,x)=>sum+x.total,0));
  readonly openCount=computed(()=>this.invoices().filter(x=>x.status==='sent').length);
  readonly visibleInvoices=computed(()=>{
    const rows=this.invoices();
    if(this.filter==='open')return rows.filter(x=>x.status==='sent');
    if(this.filter==='paid')return rows.filter(x=>x.status==='paid');
    return rows;
  });
