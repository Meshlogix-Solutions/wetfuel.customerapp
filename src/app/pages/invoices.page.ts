...
@for(invoice of visibleInvoices();track invoice.id){<ion-card class="wf-card" [routerLink]="['/invoice-detail',invoice.id]"><ion-card-content><div class="row-between"><span class="pill" [class]="invoice.statusTone">{{invoice.statusLabel}}</span><strong>{{invoice.invoiceNumber}}</strong></div><h2>{{invoice.total|currency}}</h2><p class="caption">{{invoice.lineItems.length}} deliver{{invoice.lineItems.length===1?'y':'ies'}}</p><div class="detail-row"><span>Issued {{invoice.issueDate|date:'mediumDate'}}</span><strong>Due {{invoice.dueDate|date:'mediumDate'}}</strong></div></ion-card-content></ion-card>}
