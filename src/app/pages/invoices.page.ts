...
export class InvoicesPage {
  ...
  readonly outstandingTotal=computed(()=>this.invoices().filter(x=>x.status==='sent'||x.status==='payment_pending').reduce((sum,x)=>sum+x.total,0));
  readonly openCount=computed(()=>this.invoices().filter(x=>x.status==='sent'||x.status==='payment_pending').length);
  readonly visibleInvoices=computed(()=>{
    const rows=this.invoices();
    const f=this.filter();
    if(f==='open')return rows.filter(x=>x.status==='sent'||x.status==='payment_pending');
