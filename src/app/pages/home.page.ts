...
export class HomePage {
  ...
  readonly activeOrderCount=computed(()=>this.orders().filter(x=>!['completed','cancelled'].includes(x.status)).length);
  readonly completedCount=computed(()=>this.jobs().filter(x=>x.status==='completed').length);
  readonly attentionCount=computed(()=>this.equipment().filter(x=>x.status==='active'&&Number(x.estimatedLevelPercent??0)<=35).length);
