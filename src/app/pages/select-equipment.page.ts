...
export class SelectEquipmentPage{
  ...
  readonly availableEquipment=computed(()=>{
    ...
    return this.equipment()
      .filter(x=>x.isActive&&(!siteId||x.siteId===siteId))
  ...
  remainingGallons(item:CustomerEquipment):number{return Math.round((item.capacityGallons||0)*(item.estimatedLevelPercent||0)/100);}
}
