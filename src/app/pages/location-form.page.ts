...
export class LocationFormPage{
  private readonly api=inject(CustomerApiService);private readonly router=inject(Router);saving=false;error='';
  form={name:'',address:'',city:'',state:'',zipCode:'',contactName:'',contactPhone:'',isDefault:false};
  async save():Promise<void>{if(!this.form.name.trim()||!this.form.address.trim()||!this.form.city.trim()||this.form.state.trim().length!==2||!this.form.zipCode.trim()){this.error='Complete the required address fields.';return;}this.saving=true;this.error='';try{await firstValueFrom(this.api.addSite({...this.form,contactName:this.form.contactName||undefined,contactPhone:this.form.contactPhone||undefined}));await this.router.navigateByUrl('/locations');}catch{this.error='The location could not be saved.';}finally{this.saving=false;}}
}
