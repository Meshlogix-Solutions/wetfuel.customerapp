...
@if(invoice();as x){
  <ion-card class="wf-card hero-card"><ion-card-content><span class="pill dark">{{x.statusLabel}}</span><h2 style="font-size:38px;margin:14px 0 4px">{{x.total|currency}}</h2><p class="caption" style="margin:0">Issued {{x.issueDate|date:'mediumDate'}} · Due {{x.dueDate|date:'mediumDate'}}</p></ion-card-content></ion-card>
  ...
  <ion-card class="wf-card"><ion-card-content><div class="detail-row"><span>Customer</span><strong>{{x.customerName}}</strong></div><div class="detail-row"><span>Status</span><strong>{{statusDetail(x)}}</strong></div></ion-card-content></ion-card>
  @if(x.canPay){<ion-button class="wf-button" expand="block" [disabled]="paying" (click)="pay(x)"><ion-icon name="card-outline" slot="start"></ion-icon>{{paying?'Processing...':'Pay '+(x.total|currency)}}</ion-button>}
...
export class InvoiceDetailPage {
  ...
  statusSubtitle():string{
    const x=this.invoice();if(!x)return 'Loading...';
    switch(x.statusGroup){
      case 'paid':return 'Paid';
      case 'processing':return 'Payment Processing';
      case 'overdue':return 'Overdue';
      default:return 'Open · Due '+new Date(x.dueDate||'').toLocaleDateString();
    }
  }
  statusDetail(x:CustomerInvoice):string{
    switch(x.statusGroup){
      case 'paid':return 'Paid'+(x.paidDate?' '+new Date(x.paidDate).toLocaleDateString():'');
      case 'processing':return 'Confirming with QuickBooks';
