...
@Component({
  ...
  standalone: true,
  template: `<div class="heading"><strong>Delivery territory</strong><div>The map is centered on the selected address. Tap around the territory boundary; at least 3 points are required.</div></div>
...
export class CustomerMapboxTerritoryEditorComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() siteLatitude?:number|null;
  @Input() siteLongitude?:number|null;
  ...
  async ngAfterViewInit():Promise<void>{
    ...
    const sitePoint=this.getSitePoint();
    this.map=new mapboxgl.Map({accessToken:environment.mapboxAccessToken,container:this.mapElement.nativeElement,style:rasterStyle(environment.mapboxAccessToken),projection:'mercator',center:this.points.length?[this.points[0][0],this.points[0][1]] as [number,number]:sitePoint??[-98.5795,39.8283],zoom:this.points.length?10:sitePoint?14:3});
  ...
  ngOnChanges(changes:SimpleChanges):void{if((changes['territoryGeoJson']||changes['siteLatitude']||changes['siteLongitude'])&&!this.syncing){if(changes['territoryGeoJson'])this.readInput();if(this.ready){this.render(false);if(changes['siteLatitude']||changes['siteLongitude'])this.focusSite();else this.scheduleViewportSync();}}}
  ...
  private draw():void{if(!this.map)return;const canvas=this.overlayElement.nativeElement,mapCanvas=this.map.getCanvas(),width=mapCanvas.clientWidth,height=mapCanvas.clientHeight,ratio=window.devicePixelRatio||1;if(canvas.width!==Math.round(width*ratio)||canvas.height!==Math.round(height*ratio)){canvas.width=Math.round(width*ratio);canvas.height=Math.round(height*ratio);}canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;const context=canvas.getContext('2d');if(!context)return;context.setTransform(ratio,0,0,ratio,0,0);context.clearRect(0,0,width,height);const projected=this.points.map(p=>this.map!.project(p as [number,number]));if(projected.length>=2){context.beginPath();context.moveTo(projected[0].x,projected[0].y);projected.slice(1).forEach(p=>context.lineTo(p.x,p.y));if(projected.length>=3){context.closePath();context.fillStyle='rgba(227,27,35,.28)';context.fill();}context.strokeStyle='#e31b23';context.lineWidth=4;context.lineJoin='round';context.stroke();}for(const p of projected){context.beginPath();context.arc(p.x,p.y,8,0,Math.PI*2);context.fillStyle='#e31b23';context.fill();context.strokeStyle='#fff';context.lineWidth=3;context.stroke();}const site=this.getSitePoint();if(site){const p=this.map.project(site);context.beginPath();context.arc(p.x,p.y,11,0,Math.PI*2);context.fillStyle='#2563eb';context.fill();context.strokeStyle='#fff';context.lineWidth=4;context.stroke();context.beginPath();context.arc(p.x,p.y,3,0,Math.PI*2);context.fillStyle='#fff';context.fill();}}
  private fit():void{if(!this.map)return;const locations=[...this.points];const site=this.getSitePoint();if(site)locations.push(site);if(!locations.length)return;if(locations.length===1){this.map.jumpTo({center:locations[0] as [number,number],zoom:14});return;}const bounds=locations.reduce((b,p)=>b.extend(p as [number,number]),new this.mapbox!.LngLatBounds());this.map.fitBounds(bounds,{padding:45,maxZoom:14,duration:0});}
  private focusSite():void{const point=this.getSitePoint();if(this.map&&point)this.map.flyTo({center:point,zoom:14,duration:500});}
  private getSitePoint():[number,number]|null{if(this.siteLatitude==null||this.siteLongitude==null)return null;const latitude=Number(this.siteLatitude),longitude=Number(this.siteLongitude);return Number.isFinite(latitude)&&Number.isFinite(longitude)?[longitude,latitude]:null;}
